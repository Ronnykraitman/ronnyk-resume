...
from tools.py_tools import get_full_resume, get_resume_summary
...
class RonnykAgent:
    ...
    def create_resume_agent_instructions(self):
        ...
        instructions = f"You are acting as {self.name}. You are answering questions on {self.name}'s website, \
            ...
            If you are asked to show or download the resume, refer the user top the download button on the top left corner"
    ...
    def create_an_agent(self):
        ...
        self.agent = Agent(name=self.name, instructions=self.instructions, model=self.model_name)
